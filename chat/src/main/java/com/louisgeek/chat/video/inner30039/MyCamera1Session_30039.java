 ...
 */
public class MyCamera1Session_30039 {
...
//        camera1ResolutionHistogram = Histogram.createEnumeration("WebRTC.Android.Camera1.Resolution", CameraEnumerationAndroid.COMMON_RESOLUTIONS.size());
    ...
    private MyCamera1Session_30039(Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, Camera camera, Camera.CameraInfo info, CameraEnumerationAndroid.CaptureFormat captureFormat, long constructionTimeNs) {
    ...
    public static void create(CreateSessionCallback callback, Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, int width, int height, int framerate) {
        ...
        if (camera == null) {
            ...
            callback.onDone(new MyCamera1Session_30039(events, captureToTexture, applicationContext, surfaceTextureHelper, cameraId, camera, info, captureFormat, constructionTimeNs));
        }
    }
    ...
    private static CameraEnumerationAndroid.CaptureFormat findClosestCaptureFormat(Camera.Parameters parameters, int width, int height, int framerate) {
        List<CameraEnumerationAndroid.CaptureFormat.FramerateRange> supportedFramerates = MyCamera1Enumerator_30039.convertFramerates(parameters.getSupportedPreviewFpsRange());
        ...
        Size previewSize = CameraEnumerationAndroid.getClosestSupportedSize(MyCamera1Enumerator_30039.convertSizes(parameters.getSupportedPreviewSizes()), width, height);
//        reportCameraResolution(camera1ResolutionHistogram, previewSize);
    ...
    private static Size findClosestPictureSize(Camera.Parameters parameters, int width, int height) {
        return CameraEnumerationAndroid.getClosestSupportedSize(MyCamera1Enumerator_30039.convertSizes(parameters.getSupportedPictureSizes()), width, height);
    }
    ...
    private void startCapturing() {
        ...
        this.camera.setErrorCallback(new Camera.ErrorCallback() {
            ...
            public void onError(int error, Camera camera) {
                ...
                stopInternal();
                if (error == 2) {
                    events.onCameraDisconnected(MyCamera1Session_30039.this);
                } else {
                    events.onCameraError(MyCamera1Session_30039.this, errorMessage);
                }
...
//                        camera1StartTimeMsHistogram.addSample(startTimeMs);
                    ...
                    events.onFrameCaptured(MyCamera1Session_30039.this, frame);
                    frame.release();
                }
            }
        });
    ...
    public interface Events {
        ...
        void onCameraError(MyCamera1Session_30039 var1, String var2);

        void onCameraDisconnected(MyCamera1Session_30039 var1);

        void onCameraClosed(MyCamera1Session_30039 var1);

        void onFrameCaptured(MyCamera1Session_30039 var1, VideoFrame var2);
    }
    ...
    public interface CreateSessionCallback {
        void onDone(MyCamera1Session_30039 var1);
